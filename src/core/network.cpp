...
NeuralNetwork::NeuralNetwork(int64_t input_channels, int64_t num_filters,
  ...
  batch_norm =
      register_module("batch_norm", torch::nn::BatchNorm2d(num_filters));

  policy_fc = register_module(
      "policy_fc", torch::nn::Linear(num_filters * board_size_, num_actions));
...
NeuralNetwork::NeuralNetwork(const Config& config)
    : NeuralNetwork(3, config.num_filters, config.action_size,
                    config.num_residual_blocks) {}

std::pair<torch::Tensor, torch::Tensor> NeuralNetwork::forward(
    torch::Tensor x) {
