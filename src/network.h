...
inline std::shared_ptr<NeuralNetwork> CreateNetwork(const Config& config) {
  return std::make_shared<NeuralNetwork>(config, GameType::GetBoardSize(), GameType::GetActionSize());
}
